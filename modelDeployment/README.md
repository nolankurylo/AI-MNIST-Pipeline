...
```bash
modelDeployment$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
modelDeployment$ sam local start-api
modelDeployment$ curl http://localhost:3000/classify_digit
```
...
```bash
modelDeployment$ sam logs -n InferenceFunction --stack-name modelDeployment --tail
```
...
```bash
aws cloudformation delete-stack --stack-name modelDeployment
```
